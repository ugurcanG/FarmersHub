  ...
  <div class="row q-col-gutter-md">
    ...
    <div class="col-12 col-md-6">
      ...
      <q-table
        ...
        :columns="[
          { name: 'label', label: '', align: 'left', field: row => row[1], style: 'font-weight: bold;' },
          { name: 'value', label: '', align: 'left', field: row => formatValue((field as Record<string, unknown>)[row[0]], row[0]) }
        ]"
      />
    </div>
    ...
    <div class="col-12 col-md-6">
      ...
      <q-table
        ...
        :columns="[
          { name: 'label', label: '', align: 'left', field: row => row[1], style: 'font-weight: bold;' },
          { name: 'value', label: '', align: 'left', field: row => formatValue(averageValues[row[0]], row[0]) }
        ]"
      />
    </div>
  </div>
...
const averageValues = computed<Record<string, string>>(() => {
  ...
  const sum = (key: keyof FieldMeasurement) =>
    fieldMeasurements.value.reduce((acc, m) => acc + (Number(m[key]) || 0), 0) / fieldMeasurements.value.length;
