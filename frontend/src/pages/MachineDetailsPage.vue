  ...
  <q-page class="q-pa-md">
    ...
    <q-card class="q-pa-md shadow-2">
      ...
      <q-card-section>
        ...
        <p>
          <strong>Zugewiesenes Feld:</strong>
          {{ machine.assigned_field?.name ?? 'Kein Feld zugewiesen' }}
        </p>
        <p>
...
const renderCharts = () => {
  ...
  const lineData = measurements.value.map((m) =>
    Number(m[selectedLineData.value?.value as keyof MachineMeasurement] ?? 0),
  )
  const barData = measurements.value.map((m) =>
    Number(m[selectedBarData.value?.value as keyof MachineMeasurement] ?? 0),
  )
