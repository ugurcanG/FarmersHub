  ...
  <q-dialog v-model="localShowModal">
    <q-card>
      ...
      <q-card-section>
        ...
        <q-select v-model="machineData.category" :options="categoryOptions" label="Kategorie" filled />
      </q-card-section>
...
const { showModal, machineToEdit } = defineProps({
  showModal: Boolean,
  machineToEdit: {
    type: Object as PropType<{ name: string; serial_number: string; year_of_manufacture: number; status: string; category: string } | null>,
    default: null,
  },
})
...
watch(
  ...
  (newMachine) => {
    if (newMachine) {
      ...
      machineData.value = { name: '', serial_number: '', year_of_manufacture: null, status: 'In Betrieb', category: 'Traktor' }
    }
  },
  { immediate: true },
)
...
const submitMachine = () => {
  ...
  if (!machineData.value.year_of_manufacture || machineData.value.year_of_manufacture < 1900 || machineData.value.year_of_manufacture > new Date().getFullYear()) {
